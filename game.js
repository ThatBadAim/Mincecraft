...
class GameController {
  ...
  buildInventoryGridUI() {
    ...
    for (let i = 0; i < 27; i++) {
      ...
      slotEl.addEventListener('click', () => this.handleInventorySlotClick(i));
  ...
  handleInventorySlotClick(index) {
    if (this.selectedInventorySlotIndex === null) {
      ...
      // Swap items
      const temp = this.inventorySlots[this.selectedInventorySlotIndex];
      this.inventorySlots[this.selectedInventorySlotIndex] = this.inventorySlots[index];
  ...
  updateRaycastTarget() {
    ...
    const intersects = this.raycaster.intersectObjects(Array.from(this.world.meshList));
